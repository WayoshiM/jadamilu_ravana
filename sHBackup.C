...
//									std::cout << prod1 << std::endl << std::endl; 
						...
						if(js == is || UH[js + is*slice_dim])
							ja_a_size++;
					}
				}
				
				//	create parameters for DPJD
				printf("Creating parameters for DPJD...\n\n");
				  ...
				  for (long int j=0; j<slice_dim; j++) {
					...
					ia[slice_dim] = ja_a_counter;
					
					printf("ia: ");
				  for(int i = 0; i < slice_dim+1; i++) printf("%d ", ia[i]);
				  printf("\nja: ");
				  for(int i = 0; i < ja_a_size; i++) printf("%d ", ja[i]);
				  printf("\na: ");
				  for(int i = 0; i < ja_a_size; i++) printf("%f ", a[i]);
				  ...
				  std::cout << "Setting up " << slice_dim << " took " << dur2 <<" s, or " << dur2/60 << " mins" << std::endl;             
                  std::cout << "Just diagonalizing " << slice_dim << " took " << dur1 <<" s, or " << dur1/60 << " mins" << std::endl << std::endl; 
