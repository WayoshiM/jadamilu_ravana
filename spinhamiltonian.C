		...
		switch (mat_type) {
			...
			case 3:
			  // a {} block is created due to scope errors otherwise
			  {
				...
				std::vector<int> ja_vector(0);
				std::vector<double> a_vector(0);
...
//									std::cout << prod1 << std::endl << std::endl; 
									...
									value += prod1 + prod2;
									//on the diagonal, we want to do this twice
									if(is == js)
										value += prod1 + prod2;
								}
						...
						if(is == js || value) {
							if(is == js)
								ia[is] = ja_a_counter+1;
							ja_vector.push_back(js+1);
				...
				printf("Creating remaining parameters for DPJD...\n\n");
				  ...
				  dpjd_(&n, &a_vector[0], &ja_vector[0], ia, eigs, res, x, &lx, &neig, &sigma, &isearch, &ninit,
